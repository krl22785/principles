...
def kdtreeApproach(tripLocations, startRectangle, endRectangle):
    ...
    # # #find radius and centroid for end rectangle
    # end_lat = endRectangle[0][1] - endRectangle[0][0]
    # end_lon = endRectangle[1][1] - endRectangle[1][0]
    # radius2 = math.sqrt((end_lat/2) ** 2 + (end_lon/2) ** 2)
    #
    # centroid2 = ((endRectangle[0][0] + end_lat/2), (endRectangle[1][0] + end_lon/2))
    # tree2 = spatial.KDTree([(i[2],i[3]) for i in tripLocations])
    #
    # #build tree 2
    # #tree2 = spatial.KDTree([(tripLocations[i][2], tripLocations[i][3]) for i in ind]) ## WHY IS THIS NOT WORKING !!!!!!
    # tree2 = spatial.KDTree([(i[2],i[3]) for i in tripLocations])
    # ind2 = tree2.query_ball_point(centroid2, radius2)
    #
    # final_ind = set(ind).intersection(ind2)
    #
    # for i in final_ind:
    #     if tripLocations[i][0] <= 40.721319 and tripLocations[i][0] >= 40.713590 and tripLocations[i][1] >= -74.011116 and tripLocations[i][1] <= -73.994722:
    #         if tripLocations[i][2] <= 40.748398 and tripLocations[i][2] >= 40.744532 and tripLocations[i][3] >= -74.003005 and tripLocations[i][3] <= -73.990881:
